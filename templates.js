    ...
    <div class="text-scene-settings">
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                ...
                <input id="ts-time-tokens" class="text_pole" type="number" min="200" max="20000" step="200">

                <label for="ts-bridge-turns">${t("상황 연결 (메인 채팅 최근 N개 메시지)")}</label>
                ...
                <label class="checkbox_label" for="ts-typing-effect">
                    ...
                    <span>${t("답장을 여러 버블로 나눠서 천천히 받기")}</span>
                </label>

                <label for="ts-default-mode">${t("종료할 때 기본 형식")}</label>
                <select id="ts-default-mode" class="text_pole">
                    ${Object.values(INSERT_MODE).map(v =>
                        `<option value="${v}">${INSERT_MODE_LABELS[v]}</option>`).join('')}
                </select>

                <label for="ts-insert-lang">${t("메인 채팅에 넣을 때 언어")}</label>
                ...
                <div class="inline-drawer">
                    ...
                    <div class="inline-drawer-content">
                        <label for="ts-messenger-prompt">${t("메신저 지시문")}</label>
                        <textarea id="ts-messenger-prompt" class="text_pole textarea_compact" rows="10"></textarea>

                        <label for="ts-summary-prompt">${t("요약 지시문")}</label>
                        <small class="ts-note"><code>{{log}}</code> ${t("자리에 문자 내용이 들어갑니다.")}</small>
                        ...
                        <div class="ts-settings-actions">
                            <button id="ts-reset-prompts" class="menu_button">
                                <i class="fa-solid fa-rotate-left"></i> ${t("프롬프트 기본값으로")}
                            </button>
                        </div>
                    </div>
                </div>
