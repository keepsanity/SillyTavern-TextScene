...
export function renderBubble(message, session) {
    ...
    <div class="ts-row ts-row-${side}${message.inserted ? ' ts-row-inserted' : ''}" data-id="${escapeHtml(message.id)}">
        ...
        <div class="ts-meta">
            ${unread ? '<span class="ts-unread" title="아직 읽지 않음">1</span>' : ''}
...
function renderGapNote(minutes) {
    ...
    if (m < 60) label = `${m}분 뒤`;
        ...
        const rest = m % 60;
        label = rest ? `${h}시간 ${rest}분 뒤` : `${h}시간 뒤`;
    } else {
        label = `${Math.floor(m / 1440)}일 뒤`;
    }
...
export function renderBubbleList(session) {
    if (!session?.messages?.length) {
        return `<div class="ts-empty">아직 주고받은 문자가 없어요.</div>`;
    }
    ...
    for (const message of session.messages) {
        if (!markedCommitLine && !message.inserted && hasInserted(session)) {
            parts.push(`<div class="ts-commit-line"><span>여기까지 RP 에 반영됨</span></div>`);
...
function hasInserted(session) {
    return (session?.messages ?? []).some(m => m.inserted);
}
